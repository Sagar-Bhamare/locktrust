...
const EditAppDialog = ({ open, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      // Create a map of all modules with isDefault status based on existing data
      const moduleStatus = allModules.map(module => ({
        name: module,
        isDefault: initialData.modules.some(m => m.name === module && m.isDefault)
      }))
  ...
  const selectedCount = formData.modules?.filter(m => m.isDefault).length || 0

  return (
    <Dialog open={open} onClose={onClose} maxWidth='md' fullWidth>
      <DialogTitle>
        <Box display='flex' justifyContent='space-between' alignItems='center'>
          <Typography variant='h5' sx={{ fontWeight: 600 }}>Edit App</Typography>
